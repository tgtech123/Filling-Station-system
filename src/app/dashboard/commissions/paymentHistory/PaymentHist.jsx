...
const PaymentHist = () => {
  ...
  const { paymentHistory, loading, errors, fetchPaymentHistory } = useCommissionStore();

  useEffect(() => {
    fetchPaymentHistory();
  }, [fetchPaymentHistory]);

  const paymentData = getPaymentData(paymentHistory);
    ...
    <div className="mt-5 px-2 sm:px-4 lg:px-6">
      ...
      {loading.paymentHistory && paymentHistory.length === 0 ? (
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-800">Error: {errors.paymentHistory}</p>
          <button
