...
const ActivateActionModal = ({ isOpen, onClose, data }) => {
  ...
  const handleActivate = async () => {
    setIsLoading(true);
    try {
      const res = await updateStationStatus(stationId, true);
      if (res.success) {
        ...
        <div>
          <div className="flex justify-between">
            <div className="mt-[1rem] mb-[2rem]">
              <div className="flex gap-6">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <button className="bg-[#FEF2E5] text-[#F57C00] w-fit px-3 h-[29px] font-semibold text-[14px] rounded-2xl">
                    Suspended
                  </button>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <p className="text-neutral-500 text-[1rem] font-medium">Maintenance mode</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 mb-4">
            ...
            <span className="flex gap-3">
              ...
              <h1 className="text-[1rem] font-medium text-[#616161]">
                Registered{" "}
                {data?._raw?.createdAt
                  ? new Date(data._raw.createdAt).toLocaleDateString("en-US", { month: "short", year: "numeric" })
                  : "—"}
              </h1>
            </span>
          </div>
          ...
          <div className="border-b-[2px] border-gray-200 lg:p-4 p-2">
            ...
            <div className="mt-[1rem] grid lg:grid-cols-2 grid-cols-1">
              ...
              <span className="flex flex-col mb-[1rem] gap-1">
                ...
                <span className="text-[1rem] font-semibold bg-[#FEF2E5] rounded-2xl w-fit p-2 h-[32px] text-[#F57C00] text-center flex items-center justify-center">
                  Suspended
                </span>
              </span>
              <span className="flex flex-col gap-1">
          ...
          <div className="mt-[1.235rem] flex flex-col gap-4">
            <button
              ...
              disabled={isLoading}
              className="flex gap-2 items-center cursor-pointer justify-center text-[#1A71F6] border-[1px] border-[#1A71F6] w-full py-2 rounded-full text-[0.875rem] font-medium disabled:opacity-50"
            >
              <Play size={18} />
              {isLoading ? "Activating..." : "Activate Station"}
            </button>
            <button
              onClick={() => setIsResetOpen(true)}
              className="flex gap-2 items-center cursor-pointer justify-center border-[1px] border-[#D0D5DD] w-full py-2 rounded-full text-[0.875rem] font-medium"
            >
              <MdLockReset size={18} />
