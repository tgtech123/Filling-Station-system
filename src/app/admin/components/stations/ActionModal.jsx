...
const ActionModal = ({ isOpen, onClose, data }) => {
  ...
  // Read from the nested response shape
  ...
  const handleSuspend = async () => {
    ...
    try {
      const res = await updateStationStatus(station.id || stationId, false);
      if (res.success) setIsSuspendOpen(true);
  ...
  const handleRestore = async () => {
    setIsRestoring(true);
    try {
      const res = await updateStationStatus(station.id || stationId, true);
      if (res.success) toast.success(`"${stationName}" restored successfully`);
  ...
  const handleResetPassword = async () => {
    setIsResetting(true);
    try {
      const res = await resetStaffPassword(station.id || stationId, owner.id);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await deleteStation(station.id || stationId);
      if (res.success) {
        ...
        <div className="border-t-[2px] border-b-[2px] border-gray-200 p-4 mb-0">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-y-4">
            <span className="flex flex-col gap-1">
              ...
              <span className="text-[0.895rem] font-semibold">
                {owner.firstName || "—"}
              </span>
            </span>
            <span className="flex flex-col gap-1">
        ...
        <div className="border-b-[2px] border-gray-200 p-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-y-4">
            <span className="flex flex-col gap-1">
              ...
              <span
                className={`text-[0.875rem] font-semibold rounded-2xl w-fit px-3 h-[32px] flex items-center ${planBadge(
            ...
            <span className="flex flex-col gap-1">
              ...
              <span
                className={`text-[0.875rem] font-semibold rounded-2xl w-fit px-3 h-[32px] flex items-center ${
            ...
            {isDeleted ? (
              <button
                onClick={async () => {
                  ...
                  try {
                    const res = await restoreStation(station.id || stationId);
                    if (!res.success) toast.error(res.error || "Failed to restore station");
                  } catch {
                ...
                className="flex gap-2 items-center justify-center cursor-pointer text-green-700 border border-green-500 w-full py-2 rounded-lg text-[0.875rem] font-medium disabled:opacity-50 hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors"
              >
                <RotateCcw size={18} />
